...
var Director = (function(){
  ...
  var VOICES = {
    julien:{name:'于连·索雷尔',color:'#ef344c'},
    renale:{name:'德·雷纳尔夫人',color:'#edb9c4'},
    mathilde:{name:'玛蒂尔德',color:'#d7b04d'},
    mayor:{name:'德·雷纳尔先生',color:'#9fb1c5'},
    abbe:{name:'谢朗神父',color:'#b7adb8'},
    judge:{name:'审判长',color:'#d1c7b4'},
    chorus:{name:'众声',color:'#d6cbd5'},
    band:{name:'红黑乐队',color:'#ef344c'},
    narrator:{name:'',color:'#b9a995'}
  };
  ...
  function draw(dt){
    ...
    if(fin){
      ...
      ctx.globalAlpha = fa;
      txtOutline(ctx, 'F I N', G.RW/2, G.RH/2-30, 26, '#eee8df', '#820f22', 'center', true, FONT_SERIF);
      txt(ctx, '红与黑 — 全剧终', G.RW/2, G.RH/2+8, 10, '#a08898', 'center');
      if(fin.t>2){
