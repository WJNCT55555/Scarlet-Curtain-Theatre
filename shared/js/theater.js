...
var Theater = (function () {
    ...
    var styleName = requestedStyle === 'blackbox' || requestedStyle === 'electric' ? requestedStyle : 'proscenium';
    var profiles = {
    ...
    function cyl(rt, rb, h, seg, c) { return new THREE.Mesh(new THREE.CylinderGeometry(rt, rb, h, seg || 8), mat(c)); }
    function cone(r, h, seg, c) { return new THREE.Mesh(new THREE.ConeGeometry(r, h, seg || 8), mat(c)); }
    function sph(r, c, ws, hs) { return new THREE.Mesh(new THREE.SphereGeometry(r, ws || 8, hs || 6), mat(c)); }
    function plane(w, h, c, opts) { return new THREE.Mesh(new THREE.PlaneGeometry(w, h), flat(c, opts)); }
    function init(canvas) {
        ...
        if (styleName === 'electric') {
            ...
            lights.electricL.position.set(-90, 45, -20);
            scene.add(lights.electricL);
            lights.electricR = new THREE.DirectionalLight('#ef2948', 0.42);
            lights.electricR.position.set(90, 35, 10);
            scene.add(lights.electricR);
        }
        /* 追光灯 ×2 (克制强度 + 可见光柱道具) */
        function makeSpot(color) {
            ...
            scene.add(beam);
            /* 台面光池 */
            var pool = new THREE.Mesh(new THREE.CircleGeometry(8.5, 14), flat(color, { transparent: true, opacity: 0, depthWrite: false }));
    ...
    function shot(o) {
        if (o.pos) {
            ...
            cam.tPos.set(o.pos[0], o.pos[1], o.pos[2]);
        }
        else {
            cam.fromPos.copy(cam.pos);
            cam.tPos.copy(cam.pos);
        }
        if (o.look) {
            cam.fromLook.copy(cam.look);
            cam.tLook.set(o.look[0], o.look[1], o.look[2]);
        }
        else {
            cam.fromLook.copy(cam.look);
            cam.tLook.copy(cam.look);
        }
        cam.fromFov = cam.fov;
        ...
        if (!cam.dur) {
            cam.pos.copy(cam.tPos);
            cam.look.copy(cam.tLook);
            cam.fov = cam.tFov;
            cam.t = 1;
        }
        else
        ...
        if (o.drift !== undefined)
            cam.drift = o.drift * profile.driftScale;
    }
    /* ============================================================
       灯光 DSL
       ============================================================ */
    ...
    function mood(o, dur) {
        dur = dur || 1.5;
        if (o.amb !== undefined)
            tweenLight(lights.amb, 'intensity', o.amb * profile.ambScale, dur);
        if (o.ambColor)
            tweenLight(lights.amb, 'color', styledColor('amb', o.ambColor), dur);
        if (o.hemi !== undefined)
            tweenLight(lights.hemi, 'intensity', o.hemi * profile.hemiScale, dur);
        if (o.key !== undefined)
            tweenLight(lights.key, 'intensity', o.key * profile.keyScale, dur);
        if (o.keyColor)
            tweenLight(lights.key, 'color', styledColor('key', o.keyColor), dur);
        if (o.rim !== undefined)
            tweenLight(lights.rim, 'intensity', o.rim * profile.rimScale, dur);
        if (o.rimColor)
            tweenLight(lights.rim, 'color', styledColor('rim', o.rimColor), dur);
        if (o.foot !== undefined)
            tweenLight(lights.foot, 'intensity', o.foot * profile.footScale, dur);
        if (o.footColor)
            tweenLight(lights.foot, 'color', styledColor('foot', o.footColor), dur);
        if (o.keyPos)
            lights.key.position.set(o.keyPos[0], o.keyPos[1], o.keyPos[2]);
    }
    ...
    function spot(which, o) {
        ...
        if (o.follow !== undefined)
            s.follow = o.follow;
        if (o.pos)
            s.light.position.set(o.pos[0], o.pos[1], o.pos[2]);
        if (o.color) {
        ...
        if (o.angle !== undefined)
            s.light.angle = clamp(o.angle * profile.angleScale, 0.14, 0.5);
        if (o.penumbra !== undefined)
            s.light.penumbra = o.penumbra;
        if (o.at)
            s.target.position.set(o.at[0], o.at[1], o.at[2]);
        if (o.ty !== undefined)
            s.ty = o.ty;
    }
    /* ============================================================
